"""
MAIN.PY - this is the file that defines & runs the RL algorithms for Single/Multi Agent
"""
...
def make_env(rank):
    ...
    def _init():
        env = SumoEnvironment(
            net_file=os.path.dirname(__file__) + '/../nets/2way-single-intersection/single-intersection.net.xml',
            route_file=os.path.dirname(__file__) + '/../nets/2way-single-intersection/single-intersection-vhvh.rou.xml',
            out_csv_name='output/waiting_times{}'.format(rank), use_gui=True, num_seconds=100000, max_depart_delay=0,
            single_agent=True)
        return env
    return _init
...
if __name__ == '__main__':
    ...
    num_cpu = 2  # Number of multi agent
    ...
    if args.method == 'ppo':
        ...
        env = SumoEnvironment(
            net_file=os.path.dirname(__file__) + '/../nets/2way-single-intersection/single-intersection.net.xml',
            route_file=os.path.dirname(__file__) + '/../nets/2way-single-intersection/single-intersection-vhvh.rou.xml',
            out_csv_name='output/waiting_times', use_gui=False, num_seconds=100000, max_depart_delay=0,
            single_agent=True)
